...
void transfer_server_client(char nume_fis[],char buff[]);// folosesc functia asta pentru a transfera site-ul de la client la server cand foloseste comanda add
void transfer(char buf[],char nume[]);//folosesc functia asta pentru a transfera site-ul de la server la client cand clientul foloseste get
int verificare_fisier_html(char  buff[]);//folosesc functia asta pentru a verifica daca fisierul dat este in baza de date
char *listare(char*);//cu functia asta listez toate comenzile pe care le are la dispozitie clientul cand foloseste !help 
void umplere(char nume_fisier[]);//iar cu functia asta le umplu cu informatii site-urile dupa ce le-am creat 
void fisiere_html_creare();//folosesc functia asta pentru acrea niste fisiere html simple 
int verificare1(char s[]);//verific daca numele si parola au fost trimise sub forma nume:parola cand clientul se inregistreaza cu login
int verificare_client(char nume_parola[]);//functia verifica daca clientul se afla in baza de date
...
{
    int ok=0;
    char buff[4096];//mesajul pe care il trimite serverul
	struct thData tdL; 
	tdL= *((struct thData*)arg);
    ...
    if(ok==0&&verificare1(mesaj_primit)==1)
    {
     ...
     else strcpy(buff,"Nu esti inregistrat!");
